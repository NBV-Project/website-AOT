      ...
      <AdminPortal>
        <div className={`fixed inset-0 z-[9999] flex items-center justify-center p-4 transition-all duration-500 ${saveStatus !== "idle" ? "opacity-100 visible" : "opacity-0 invisible pointer-events-none"}`}>
          ...
          <div className="relative w-full max-w-sm bg-white rounded-[2.5rem] p-10 shadow-2xl text-center">
            {saveStatus === "confirming" && <div className="space-y-8 py-2"><div><h3 className="text-xl font-bold text-slate-800">ยืนยันการบันทึก?</h3><p className="text-slate-500 mt-2 text-sm font-medium leading-relaxed px-4">บันทึกข้อมูลหน้า Contact ลงฐานข้อมูล</p></div><div className="flex gap-3"><button onClick={() => setSaveStatus("idle")} className="flex-1 py-3 rounded-2xl font-bold text-slate-400 hover:bg-slate-50 transition-colors text-sm uppercase tracking-widest">ยกเลิก</button><button onClick={handleSave} className="flex-1 py-3 bg-[#002548] text-white rounded-2xl font-bold shadow-lg hover:bg-slate-800 transition-all text-sm uppercase tracking-widest">บันทึก</button></div></div>}
            ...
            {saveStatus === "success" && <div className="space-y-8 py-2"><div><h3 className="text-xl font-bold text-slate-800">บันทึกเรียบร้อย</h3><p className="text-slate-500 mt-2 text-sm font-medium leading-relaxed px-4">หน้า Contact จะอ่านข้อมูลชุดนี้ทันที</p></div><button onClick={() => setSaveStatus("idle")} className="w-full py-3 bg-[#002548] text-white rounded-2xl font-bold shadow-lg hover:bg-slate-800 transition-all text-sm uppercase tracking-widest">ตกลง</button></div>}
