...
const ContactSupportModal = dynamic(
  () => import("./contact-support-modal").then((m) => m.ContactSupportModal),
  { ssr: false }
);

type ContactSupportButtonsProps = {
  locale: Locale;
  supportPoints: { id: string; text: string }[];
};

export function ContactSupportButtons({ locale, supportPoints }: ContactSupportButtonsProps) {
